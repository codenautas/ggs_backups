...
begin
    ...
    --if u_app is not null then  --corregir condicion
        select rol, puesto, desagregacion_geografica, edita_personal, dni, p.comuna, jsonb_build_object('provincia', provincia, 'comuna',p.comuna, 'fraccion',fraccion,'radio',radio) 
            into u_rol, u_puesto, u_des_geo, u_edita, u_dni, u_comuna, u_niv_geo 
            from ggs_backups.usuarios left join ggs_backups.personal p using(dni) left join ggs_backups.puestos using(puesto)
        ...
        case u_rol
          ...
          when 'recepcion' then
              return reg;
          when 'procesamiento' then
              return reg;
          when 'coach' then
            if u_comuna is null then
              return reg;
            else
              if u_comuna is distinct from newPer.comuna or u_comuna is distinct from oldPer.comuna then 
                raise exception 'no se puede modificar personas de otra comuna';
              end if;
            end if;
            return reg;
          when 'coordinador' then
              if not coalesce(u_edita,false) then
                  RAISE EXCEPTION 'sin permiso de editar personal';
              ...
              elsIF TG_OP='UPDATE' and u_dni= oldPer.dni and (newPer.puesto is distinct from oldPer.Puesto and oldPer.puesto is not null
                  ...
                  RAISE EXCEPTION 'no puede modificar su campo rol ni campos geograficos con dato';
              else
                  ...
                  if ok_geo then
                      ...
                      RAISE EXCEPTION 'fuera de su jerarquia geografica';
                  end if;
              end if;
          else
              RAISE EXCEPTION 'rol sin permiso especificado';
        end case;
    --else
        --???          
    --end if;
